...
import { NextImage, DataProps, useVariantSelector } from '@site/utilities/deps';
import { Button } from '@site/snippets';
import { fetchSingleProduct } from '@site/utilities/fetch-data-functions/products';

export async function getProductSingleSection(handle: string) {
  const products = await fetchSingleProduct(handle);
  return products;
}

export function ProductSingleSection(props: DataProps<typeof fetchSingleProduct>) {
